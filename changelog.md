...
- **自 09.16 及更早版本升级将触发一次强制完整重装**：清空旧配置、运行数据与外部 `/data/adb/qsc`；重启后需重新设置。此后更新按正常路径保留配置。
- 切断过程不执行旧版 `uninstall.sh`，且不删除 `modules_update`（修复 KernelSU 安装结束报 `No such file`）。
- 建议刷入本版 zip 后重启，再通过 WebUI / APP 完成配置。
...
- **APP 启动反复提示更新**：未安装事件守护时不再将本地 versionCode=0 判为可升级；「稍后」会记住当前远端版本集合，同一批不再重复提示。
- **涓流模式选项换行异常**：WebUI 改为与其它预设一致的 Chip 横排。
- **安装残留**：安装与热更新收尾后移除模块目录内仅安装期使用的 `customize.sh`、`install/`、`apk/`（仍包含于 zip）。
- **LSPosed**：关闭热重载；作用域应勾选系统框架（`system`），不可仅勾选 `android`。

### 工程

- 仓库路径与内部命名对齐重构；用户侧模块 id 仍为 `QSC_Battery`。
- 发版：正式 / 预发布 `version` 可空，按日期自动升号；`versionCode` 与日期对齐。
- 发布工作流：伴侣 APK 输出路径与 `apps/android` 对齐，修复 CI 拷贝失败。
- 切断实现：`qsc_wipe_incompatible_module` 仅删除 `modules/QSC_Battery` 与外部 qsc，保留 `modules_update`；切断线前跳过热更新。

## 2026.09.16

### 新增

- **全量盲写开关** `switch_batch_blind`（默认开）：全量写节点并在条件满足时每轮重申；关闭则只写首个成功节点后重申。WebUI「更多选项 → 冷门/实验」、APP「进阶策略 → 停充行为」可改
