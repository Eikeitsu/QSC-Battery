...
// --- customize.sh ---
{
  ...
  if (existsSync(p) && readFileSync(p, "utf8").includes("install/helpers.sh")) {
    ...
    while (helpersEnd > abortAt && (L[helpersEnd - 1].trim() === "" || L[helpersEnd - 1].startsWith("#") || L[helpersEnd - 1].startsWith("OLD_"))) {
      helpersEnd--;
    }
    ...
    const uninstallEnd = funcEnd1(L, find1(L, (l) => l.startsWith("qsc_uninstall_old_module()")));
    ...
    if (insertAt < 0) insertAt = keep.findIndex((l) => l.startsWith("QSC_INSTALL_AUTO=0"));
...
// --- qsc_switch.sh ---
{
  ...
  if (existsSync(p) && readFileSync(p, "utf8").includes("switch_prelude.sh")) {
    ...
    const mid = Math.floor((afterFull + L.length) / 2);
    const prelude = sliceJoin(L, 3, fullAt - 1); // skip shebang + header comment + common.sh source
    write(join(lib, "switch_prelude.sh"), `#!/system/bin/sh\n# switch: conf / snapshot / early exits\n${prelude}`);
    write(join(lib, "switch_charge_full.sh"), `#!/system/bin/sh\n# switch: charge_full helper\n${sliceJoin(L, fullAt, fullEnd)}`);
    write(join(lib, "switch_eval.sh"), `#!/system/bin/sh\n# switch: stop/start decision\n${sliceJoin(L, afterFull, mid)}`);
    write(join(lib, "switch_act.sh"), `#!/system/bin/sh\n# switch: apply + description\n${sliceJoin(L, mid + 1, L.length)}`);
    write(
      p,
      `#!/system/bin/sh
