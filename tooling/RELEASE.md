...
`version` 输入**可空**。空时按正式 / 预发布通道当前清单自动升号：

| 当前版本相对「当天首版」   | 结果                                |
| -------------------------- | ----------------------------------- |
| 小于当天首版（或尚无版本） | 升到当天首版 `yyyy.MM.dd`           |
| 等于或大于当天首版         | 在当前版号上 +1（如 `….2` → `….3`） |

手填仍可用：`20260916` / `2026.09.16.2`。

发版 `versionCode` 与展示版对齐：`YYYYMMDD * 100 + 修订号`（如 `2026.09.16` → `2026091601`，`.2` → `2026091602`），并与跨通道已知最大值取较大，避免回退。CI 通道仍用单调 +1。
