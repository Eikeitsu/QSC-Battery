const QscApp = {
  ...
  bindEvents() {
    document
      .getElementById("moduleEnabled")
      ?.addEventListener("change", () => this.toggleModule());
    document
      .getElementById("temperature_switch")
    ...
    document
      .querySelector('.pref[data-toggle="module"]')
    ...
    document
      ...
      ?.addEventListener("input", () => QscUi.onFontSliderChange());
  ...
  onPowerChange() {
    ...
    QscUi.renderChips(
      "stopChips",
      QSC.POWER_STOP_PRESETS,
      Number.isNaN(stop) ? "" : String(stop),
      (id) => this.selectPowerStop(id),
    );
    QscUi.renderChips(
      "startChips",
      QSC.POWER_START_PRESETS,
      Number.isNaN(start) ? "" : String(start),
      (id) => this.selectPowerStart(id),
    );
    QscUi.updateSubs();
  },
  ...
  selectPowerStop(id) {
    ...
    QscUi.renderChips("stopChips", QSC.POWER_STOP_PRESETS, id, (value) =>
      this.selectPowerStop(value),
    );
    this.onPowerChange();
    this.save();
  },
  ...
  selectPowerStart(id) {
    ...
    QscUi.renderChips("startChips", QSC.POWER_START_PRESETS, id, (value) =>
      this.selectPowerStart(value),
    );
    this.onPowerChange();
    this.save();
  },
  ...
  selectTempStop(id) {
    ...
    QscUi.renderChips("tempStopChips", QSC.TEMP_STOP_PRESETS, id, (value) =>
      this.selectTempStop(value),
    );
    QscUi.updateSubs();
    this.save();
  },
  ...
  selectTempStart(id) {
    ...
    QscUi.renderChips("tempStartChips", QSC.TEMP_START_PRESETS, id, (value) =>
      this.selectTempStart(value),
    );
    QscUi.updateSubs();
    this.save();
  },
  ...
  async save() {
    ...
    const powerStart = parseInt(
      document.getElementById("power_start").value,
      10,
    );
    const powerStopTime = parseInt(
      document.getElementById("power_stop_time").value,
      10,
    );
    ...
    const tempSwitch = document.getElementById("temperature_switch").checked
      ? 1
      : 0;
    const tempStop = parseInt(
      document.getElementById("temperature_switch_stop").value,
      10,
    );
    const tempStart = parseInt(
      document.getElementById("temperature_switch_start").value,
      10,
    );
  ...
  applySettingsToForm() {
    ...
    document.getElementById("power_stop").value =
      s.power_stop || QSC.DEFAULTS.power_stop;
    document.getElementById("power_start").value =
      s.power_start || QSC.DEFAULTS.power_start;
    document.getElementById("power_stop_time").value =
    ...
    document.getElementById("temperature_switch_stop").value =
      s.temperature_switch_stop || QSC.DEFAULTS.temperature_switch_stop;
    document.getElementById("temperature_switch_start").value =
    ...
    QscUi.renderChips(
      "tempStopChips",
      QSC.TEMP_STOP_PRESETS,
      s.temperature_switch_stop || QSC.DEFAULTS.temperature_switch_stop,
      (id) => this.selectTempStop(id),
    );
    QscUi.renderChips(
      "tempStartChips",
  ...
  async refreshStatus(showToast) {
    ...
    if (!QscApi.hasBridge()) {
      ...
      if (badge) {
        ...
        badge.textContent =
          "无法执行命令，请用 KernelSU / 支持 WebUI 的管理器打开";
      }
      return;
    }
    ...
    const [capR, tempR, offR, switchR, descR, statusR, voltR, currR, verR] =
      await Promise.all([
        ...
        QscApi.exec(
          `[ -f '${QSC.OFF_FLAG}' ] || [ -f '${QSC.MODDIR}/disable' ] && echo 1 || echo 0`,
        ),
        QscApi.exec(`[ -f '${QSC.DATADIR}/power_switch' ] && echo 1 || echo 0`),
        QscApi.exec(
          `grep '^description=' '${QSC.MODDIR}/module.prop' 2>/dev/null | cut -d= -f2-`,
        ),
        ...
        QscApi.exec(
          `grep '^version=' '${QSC.MODDIR}/module.prop' 2>/dev/null | cut -d= -f2-`,
        ),
      ]);
    ...
    if (chargeEl) {
      if (moduleOff) chargeEl.textContent = "模块关";
        ...
        chargeEl.textContent = statusMap[chargeStatus] || chargeStatus || "--";
    }
    ...
    if (currEl) {
      currEl.textContent = Number.isNaN(currRaw)
        ? "--"
        : String(
            Math.round(Math.abs(currRaw) > 10000 ? currRaw / 1000 : currRaw),
          );
    }
  ...
  async refreshLog(showToast) {
    const lines =
      document.documentElement.getAttribute("data-layout") === "dock" ? 40 : 12;
