...
安装时若选择不安装电流控制，相关脚本与 `current.jsonc` 不会写入，WebUI 也会隐藏入口。重新刷入并选择安装即可；装入后还需打开「电流控制总开关」（默认关闭）。

## 更新后配置会恢复默认吗？

更新同 ID 的 `QSC_Battery` 时，安装脚本会询问是否保留原有配置：音量上保留 `config.conf`（以及已有的 `current.jsonc`），音量下使用新版默认值；20 秒未选择时默认保留。旧版 `QSC定量停充` / `QSC定量停充_独立开关版` 会自动卸载且**不迁移**配置。
...
停充写入 `config.conf`，电流控制写入 `current.jsonc`，一般即时生效。若仍无效，看日志是否报错节点权限或机型未适配。
