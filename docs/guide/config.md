...
进深睡后会**粘滞**数轮（少读亮度）；进入息屏加强/深睡会停简介 worker。未插电不再定时强制满轮 `qsc_switch`。
...
| `desc_viewer_pkgs`              | （空）      | 逗号追加管理器包名；Magisk←`requester`，KSU/SukiSU 随机包←`ksud debug package` / `libksud.so` |
