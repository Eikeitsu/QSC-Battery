...
**安全说明**：从 `battery_current` 读取节点并写入本机存在的路径，追加 `*restrict*_cur*`；仍不写 `charge_control_limit`、`thermal_input_current` 等。写入读回不变的节点会先记失败，连续 3 次再拉黑；改配置或重启服务会清空拉黑。部分机型对电流节点敏感，若插电重启请关闭电流控制或开兼容模式。
:::
